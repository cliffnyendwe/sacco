...
def nextDayOfWeek(dow):
    date_ = date.today()
    day_of_week_indexed = calendar.weekday(date_.year, date_.month, date_.day)
    
    day_of_week = day_of_week_indexed + 1
    #date_ = datetime.datetime.today().weekday()
    diff = dow - day_of_week
    if (diff <= 0):
        diff += 7
        
    x = date_+ timedelta(days=diff)

    return x
    
def nextDayOfWeekxx(dow):
    date_ = date.today()
    day_of_week_indexed = calendar.weekday(date_.year, date_.month, date_.day)
...
def generate_incoice_date(last_invoice_date, payment_day):
    ...
    if not last_invoice_date:
        due_date  = nextDayOfWeek(payment_day)
    else:
        #next_month = last_invoice_date + timedelta(days=last_day_of_month)
        due_date = last_invoice_date + relativedelta.relativedelta(days=7)
...
def generate_invoices():   
    ...
    memberships = ChamaMembership.objects.filter(chama_account__is_closed = False, is_active = True, chama_account__payment_cycle_option = 'Weekly')
